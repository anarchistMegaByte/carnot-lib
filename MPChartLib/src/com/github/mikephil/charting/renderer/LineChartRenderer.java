...
public class LineChartRenderer extends LineRadarRenderer {
    ...
    public void drawValues(Canvas c) {
        ...
        if (mChart.getLineData().getYValCount() < mChart.getMaxVisibleCount()
            ...
            Log.i(TAG, "data sets length " + dataSets.size());
            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                for (int j = 0; j < positions.length; j += 2) {
                    ...
                    Log.i(TAG, "entry get val and max " + entry.getVal() + " " + dataSet.getYMax());
                    if (entry.getVal() == dataSet.getYMax())
    ...
    protected void drawCircles(Canvas c) {
        ...
        Log.i(TAG, "get y max " + dataSets.get(0).getYMax());
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            int minx = Math.max(dataSet.getEntryIndex(entryFrom) - diff, 0);
            int maxx = Math.min(Math.max(minx + 2, dataSet.getEntryIndex(entryTo) + 1), entryCount);
            Log.i(TAG, "max value and min value " + maxx + " " + minx);
            ...
            for (int j = minx, count = (int) Math.ceil((maxx - minx) * phaseX + minx); j < count; j++) {
                ...
                Log.i(TAG, "circle buffer values " + e.getVal() + " " + phaseY + " " + e.getVal() + " " + e.getXIndex());

                if ((e.getVal() != dataSets.get(0).getYMax())) {
                    continue;
                }
               ...
               /* Paint textPaint = new Paint();
                ...
                c.drawText("Max \n " + e.getVal(), circlesBuffer[0], circlesBuffer[1] + 40, textPaint);
*/
